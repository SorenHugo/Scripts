...
const CONFIG = {
    ...
    // 固定参数
    checkinId: '3620182',
    appId: 'wxa1b82bb18ebd7221',
    kdtId: '120531232',
    referer: 'https://servicewechat.com/wxa1b82bb18ebd7221/17/page-frame.html',
    userAgent: 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/132.0.0.0 Safari/537.36 MicroMessenger/7.0.20.1781(0x6700143B) NetType/WIFI MiniProgramEnv/Windows WindowsWechat/WMPF WindowsWechat(0x63090a13) UnifiedPCWindowsWechat(0xf254181d) XWEB/19201',
    ...
    extraDataBase: {
        ...
        client: 'weapp',
        bizEnv: 'wsc',
        uuid: 'eyTXgsqVRtoAVfN1775714040149'
    },
    // 会员中心接口固定参数
    memberCenterParams: {
        version: '2.233.4',
        kdtId: '120531232',
        onlineKdtId: '120531232',
        currentKdtId: '120531232',
...
async function checkVersion() {
    ...
    try {
        ...
        logger.userError(`版本检测请求失败：${error.message}`);
        logger.devError(`版本检测失败详情：${error.stack}`);
        process.exit(1);
    }
}
...
function parseShargeConfig() {
    const configList = [];
    const sharge = process.env.SHARGE || '';
    if (!sharge) {
        logger.userError('未配置环境变量SHARGE，脚本终止执行');
        return [];
    }

    const lines = sharge.split(/\r?\n|\r/).filter(line => line.trim());
    if (lines.length === 0) {
        logger.userError('SHARGE环境变量中无有效账号配置，脚本终止执行');
        return [];
    }
...
async function main() {
    ...
    logger.userInfo('========== 闪极签到脚本开始执行 ==========');
    const userConfigList = parseShargeConfig();
